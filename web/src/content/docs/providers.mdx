...
Volcano Agent SDK supports 100s of models from 8 providers with function calling and MCP integration. Providers can be mixed within a single workflow.
...
| **Google Gemini**        | ✅ Full          | ✅ Native (Function calling) | ✅ Native | ✅ Complete     |
...
Google's Gemini models with function calling via the Google AI Studio API.

```typescript
import { llmGemini } from "@volcano.dev/agent";
...
| Method    | Required     | Description                                                                        |
| --------- | ------------ | ---------------------------------------------------------------------------------- |
| `apiKey`  | **Required** | Google AI Studio API key from aistudio.google.com                                  |
| `baseURL` | _Optional_   | Custom API endpoint (default: https://generativelanguage.googleapis.com/v1beta)    |
...
| `options`       | _Optional_   | Model-specific parameters                |
| `clientOptions` | _Optional_   | Client behavior (e.g., rate limit retry) |
...
const gemini = llmGemini({
  ...
  clientOptions: {
    retryOnRateLimit: {
      ...
      maxDelayMs: 30000,     // Cap for backoff
    },
  },
});
```
...
Google's Gemini models with function calling via the Vertex AI API.
