...
The Model Context Protocol (MCP) enables AI agents to securely interact with external tools and data sources. Volcano Agent SDK provides first-class MCP integration with automatic tool discovery, selection, and authentication.
...
Volcano Agent SDK supports two MCP transport types:
...
```typescript
import { agent, llmOpenAI, mcpStdio, mcp } from "@volcano/agent";
...
```typescript
import { createVolcanoTelemetry } from "@volcano/agent";
...
Volcano Agent SDK supports OAuth 2.1 and Bearer token authentication per the MCP specification. Authentication can be configured at the MCP handle level or centrally at the agent level.
...
Volcano Agent SDK automatically manages MCP connections for optimal performance.
